...
//GLOBALS
$GLOBALS['DUPLICATOR_INSTALLER_VERSION'] =  '0.2.9';
...
$dbmysqlchar = (isset($_POST['dbmysqlchar']) && $_POST['dbmysqlchar'] == '1')  	? true : false;
...
<div id="content">
	...
	if ($action == 'process') {
		...
		//Link to Log
		echo "<div style='text-align:center; margin:10px;'><i>See <a href='install-log.txt' target='_blank'>install-log.txt</a> for an overview of this process.</i></div>";	
		...
		dinstaller_log("database replace characters:{$dbcharvalid}");
		dinstaller_log("database mysql charset:{$dbmysqlchar}");
		...
		mysql_query("SET wait_timeout = {$GLOBALS['MAX_TIME']}", $conn);
		
		//ENABLE DATABASE SETTINGS
		//Possible fix for utf8 characters however Causing issues in large inserts
		//mysql_query("SET NAMES 'utf8' ", $conn);   This seems to cause problems
		//trying build in php function mysql_set_charset
		if ($dbmysqlchar) {
			mysql_set_charset("{$GLOBALS['MYSQL_CHARSET']}", $conn);
		...
		if ($table_users == 0 || $table_opts == 0) {
			dinstaller_log("Notice: You may have to manually run the install-data.sql to validate data input.\n");
		}
		...
		$currdata = parse_url("http://www.tickervalue.com/featured"); 
		...
		<form id="form-installer" action="install.php" method="post" >
			...
			<table width="100%" border="0" cellspacing="2" cellpadding="2" class="table-inputs">
				...
				<tr valign="top">
					...
					<td><input type="text" name="current_url" id="current_url" value="http://www.tickervalue.com/featured" readonly="true"  class="readonly" /></td>
				</tr>
				<tr>
					<td>Install Url</td>
				...
				<tr>
					<td>Site Title</td>
					<td><input type="text" name="site_title" id="site_title" value="Featured Products" /></td>
				</tr>		
				<tr>
					<td></td>
					...
					<div style="margin:-5px 0px 0px 140px; ">
						<table cellpadding="2" class="dbtable-opts">
							...
							<tr>
								...
								<td><input type="checkbox" name="dbmysqlchar" id="dbmysqlchar" value="1" /> <label for="dbmysqlchar">Enable MySQL character set</label></td>
							</tr>							
						</table>
			...
			<div class="warning-info">
				...
				<p><b>Setup:</b> Only the package (zip file) and install.php file should be in the install directory, unless you have manually extracted the package and checked the 'Manual Package Extraction' checkbox. All other files will be OVERWRITTEN during install.  Make sure you have full backups of all your databases and files before continuing with an installation.</p>
				
				<p><b>Manual Extraction:</b> Manual extraction requires that all contents in the package are extracted to the same directory as the install.php file.  Manual extraction is only needed when your server does not support the ZipArchive extension.  Please see the online help for more details.</p>
				
				<p><b>After Install:</b>When you are done with the installation remove the install.php, install-data.sql and the install-log.txt files from your directory. 
